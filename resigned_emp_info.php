		...
		<div id="container">
			...
			<div class="col-lg-12">
				...
				<div class="panel-body top-bordered col-lg-12">
					...
					<h2 class="summary-title">Resigned Employee Records</h2>
				
					<div class="table-buttons row">
					<?php
						if($_SESSION['rights']==1){
							echo '<a href = "emp_info.php" class = "a upper-link">Active Employees</a></td>';
						}
					?>
					</div>
								...
								<?php
										...
										switch($by){
											...
											case 0:
												$str = ($_SESSION['rights']==1 OR $_SESSION['rights']==5) ? "e.emp_num='$search' 
													...
													AND e.dept_code='$_SESSION[dept_code]'";
												break;
											//<---END of modification
										}
									}else{
									...
									while($data = $dblink->get_data($qry)){
										if($data[0] != 1 && ($data[3] == 'Resigned' || $data[3] == 'Terminated')){
											...
											echo '<td>',$data[0],'</td>';							
											echo '<td align = "left"><a style = "padding-left : 15%" href="#" onclick="info(',$data[0],');">',$data[1],'</a></td>';
											echo '<td>',$data[2],'</td>';
											echo '<td>',$data[3],'</td>';
											echo '<td>',$data[4],'</td>';
											echo '<td>',$data[5],'</td>';														
