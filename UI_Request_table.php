		...
		<div id="container">
			...
			for($i = 0; $i < count($report_record) - 1; $i++)
			{
				...
				if($type == 'allR')
				{
					echo "	<div id = 'cc'>
								<div>
										<span class = 'title'><b>" . $report_record[$i]['emp_num'] . "</b> | " . strtoupper($report_record[$i]['lastname']) . ", " . $report_record[$i]['firstname'] . " [".$report_record[$i]['department'] ."]</span>";
					echo "		</div>
							</div>";
					echo "	<div class = 't'>
				...
				for($k = 0; $k < $j; $k++)
				{
					...
					{	
						$no_records++;
						if($des % 2 != 0)
							echo "	<tr style = 'background-color:#d3dfee;'>";
						else
				...
				if($report_record[$i]['cnt'] != 0 && $type == 'recO' || $type == 'allR')
					echo "	</table></div></br/>";
				
			}
			
			if($no_records == 0)
				echo '<center><font style ="color: #122AFF; font-family: courier new; font-style: normal; font-weight: bolder; font-variant: normal;
						text-indent: 4px; cursor: pointer; font-size : 40px">NO RECORDS AVAILABLE<br/>FOR THAT CATEGORIES</font></center>';
			/*
			echo "<table id = 'datatable' border = 0>";
