...
<html>
        ...
        <script type="text/javascript">
			...
			function validate1(){
				...
				var pastDate = new Date(currentTime.getTime() - 1 * 24 * 60 * 60 * 1000);
					if((fromDate>currentTime) && type=="Sick Leave"){
						alert("For Sick Leave: Application should not be filed beyond the current date!");
						return false;
						//return true;
					}
			}
    ...
    <body alink="green" vlink="green" link="green" onload="bread('<?php echo $_SERVER["PHP_SELF"];?>');">
        <div id="container">
            ...
            <div id="cc">
            	<div>
                    <span class="title">Export</span></div>
					<div class="exportFrame">
                    	<form name="iems_export" id="iems_export" method="POST">
                        	<table border="0" cellpadding="3" cellspacing="4" width="100%">
                            	...
                            	<tr>
                                	<td>Select which report to export:</td>
                                	<td>
                                        <select name="" id="" />
...
<?php
	...
	if($_SESSION['rights']==1){
		switch($_POST['export']) //Name of the dropdown
		{ //Value of input
			//START of LEAVE -----------------------------------------------------------------------------------------------------------------
			case 'export_leaveBtn': //value of option
		...
		switch($_POST['export']) //Name of the input
		{
			...
			case 'export_leaveBtn': //Value of the input
				...
				$getExcel = "SELECT date_Filed, CONCAT(e.emp_firstname,' ',e.emp_middlename,' ',e.emp_lastname), d_from, d_to, 
								...
								LEFT JOIN ems_business_units b ON b.b_id = e.b_id
